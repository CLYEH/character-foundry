...
### 4.4 其他 AI agent 輔助（選用 + 自動鏈式觸發）

除了自動觸發的 Codex review，作者或 reviewer 可**主動**用 `.claude/agents/` 下的 project-local 專家 agent 做領域深度 review：

| Agent | 範圍 | 何時自動建議 chain |
|---|---|---|
| [`engineering-code-reviewer`](.claude/agents/engineering-code-reviewer.md) | 通用 code smell / 可讀性 / 結構 | **每次 push**（pre-push hook 預設要求） |
| [`security-engineer`](.claude/agents/security-engineer.md) | auth / secrets / attack surface / threat model | Ticket file 命中 security 關鍵字（OAuth / JWT / PKCE / Authentik / `client_secret` / refresh token / scope decorator / SAST / `security-sensitive` 等） |
| [`db-optimizer`](.claude/agents/db-optimizer.md) | schema / migration safety / index / query plan | Ticket file 命中 schema-migration 關鍵字（Alembic / ALTER TABLE / ADD COLUMN / schema migration / backfill / enum column / new index 等） |

**自動鏈式觸發機制（T-062）：**

`.claude/hooks/pre-push-review.sh` 與 `.githooks/pre-push` 在 push 前會：

1. 從 branch name 抽出 `T-XXX`
2. 讀 `tickets/T-XXX-*.md`（或 `tickets/DONE/T-XXX-*.md`），略過樣板 `## OAuth scope required` section 行
3. grep security / schema 兩組關鍵字，命中就在 deny message 內**additionally direct** Claude 跑對應 subagent

Hook 自己沒能力 spawn subagent——它只是把 directive 寫進 PreToolUse deny message。實際 chain 仍由 Claude 看到 directive 後決定（這是 §0「人 = decision-maker」的延伸）。

**選用是「選用」，不是「省略」：**

這些 agent **選用**——通常用在 push PR 前自己先過一輪、或人類 reviewer 想要第二意見時。**不取代** 人類 approve，**不取代** Codex 自動 review。但 pre-push hook 對 security-sensitive / schema-migration ticket 已經把對應 subagent 標成「強烈建議」——作者可以理由充分地跳過，但跳過要在 PR thread 留書面理由。

**Source 標註：** `security-engineer.md` / `db-optimizer.md` / `engineering-code-reviewer.md` 三者都 fork 自 [`msitarzewski/agency-agents`](https://github.com/msitarzewski/agency-agents)（與專案 conventions 對齊後落地），詳細 source attribution 在各檔尾。
