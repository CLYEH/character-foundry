...
| # | Ticket | Status |
|---|---|---|
| T-080 | MCP server skeleton（FastAPI sub-app `/mcp` + Python SDK ≥ PR #2038 + dual-stack auth integration + `hello.world` smoke tool）| TODO |
...
| T-084 | MCP tool `character.create`（packaged）+ character CRUD 1:1 wraps（13 個 tool）| TODO |
...
**Dependency / parallelization：**
- **Wave A（foundation；4 張全平行）**：T-080 / T-081 / T-082 / T-083 無內部 dep，可同時起步
