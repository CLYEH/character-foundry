...
- **Wave B（核心 tool；3 張依 Wave A + T-088）**：T-080 + T-081 + T-083 + **T-088** 全部 DONE，三張 packaged tool 可開工。**T-084 ✅ DONE（2026-05-21）—— 作為 packaged-tool + 1:1 wrap 的 reference pattern。T-085 / T-086 直接抄 `app/mcp/tools/character.py`：** CRUD wrap 走 `register(MCPTool(...))` + `require_mcp_scopes` → `require_user_context` → short-lived session → `translate_agent_errors`；DTO 直接 reuse route builder（避免 REST/MCP drift）；packaged tool 走 phase-progress（共用 `app/mcp/progress.py`）+ abandon-on-failure + phase-tagged error envelope `{error, phase}` + inline-arq-pool 測試 harness（`InlineCheckpointArqPool` in `tests/mcp/tools/conftest.py`）。
