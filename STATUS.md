...
- **Wave B（核心 tool；3 張依 Wave A + T-088）**：T-080 + T-081 + T-083 + **T-088** 全部 DONE，三張 packaged tool（T-084 / T-085 / T-086）現在可開工；T-084 先行作為 pattern reference 較順，但不是 hard dep（三張之間平行）。1:1 wrap pattern（`register(MCPTool(...))` + `require_mcp_scopes` → `require_user_context` → short-lived session → `translate_agent_errors`）可直接從 `app/mcp/tools/task.py` 抄。
