...
> **Last updated:** 2026-05-14 — T-070 opened: 透過 CDP 連使用者本機真實 Chrome 測 CF Google 登入，發現 OAuth 登入在 `localhost:5173` 壞掉。`web/vite.config.ts` 的 dev proxy 只 proxy `/api`（且 target `localhost:8000` 在 containerized `pnpm dev` 裡指不到 api container，`:5173/api/health` 回 500），**沒有 `/oauth/` proxy** → SPA 的 relative `/oauth/...` navigation（Google source-init、帳密 authorize、token exchange）打到 Vite dev server 的 SPA fallback、被 router bounce 回 `/login?redirect_back=...`。CI 看不到因為 e2e 走 nginx:80 同源。`tickets/T-070-vite-dev-oauth-proxy.md` scope 修法（加 `/oauth/` proxy → `http://nginx`、`/api` target 改 `http://api:8000`，已從 container 內驗證 target 可達）；stacked blocker：就算 proxy 修好，仍會撞 T-069 wall 1（Authentik OAuth Source 的 flow 還沒在 admin UI 設）才能 end-to-end 登入。— 同日稍早 T-069 implemented：補上 T-053 §5.2 留的 dev operator provisioning 設定缺口。`planning/devops/authentik-stack.md` §5.2 OAuth Source 建立步驟補 **Authentication flow**（`default-source-authentication`，漏設則 `email_link` 匹配成功的既有 user 也登不進）+ **Enrollment flow**（`default-source-enrollment`，漏設即 dev 測試踩到的 `Source is not configured for enrollment`）；新增 §5.7「Provision a dev operator」說明真人 operator 要登入需備妥兩層 user — Authentik user（enrollment flow 設好後首登自動 enroll，會跳選 username 中介頁）+ backend `User` row（`api/app/api/deps.py::_resolve_oauth` 走 email lookup，無 row 則所有登入入口都 401，含 T-068 帳密 fallback —— 帳密 path 拿到的一樣是 Authentik token）；原 §5.7/§5.8 順移為 §5.8/§5.9，§5.9 checklist 同步加 §5.2 flow 與 §5.7 兩項，§5 開頭加 operator-pass retrofit 標記。CLI 決定：做了 `provision-operator` 子指令（`api/app/cli.py`）而非純文件化 —— `create-user` 的 `--password` 對 OAuth operator 是用不到的欄位（其登入不碰 backend `password_hash`），`provision-operator` 是 thin wrapper（內部 `_create_user(password=secrets.token_urlsafe(32))`，team lookup / `IntegrityError` 全複用），帶隨機不記錄的 hash 把「OAuth-only、JWT-login 對該 operator 等同停用」做進結構；要帳密 break-glass 仍走 `create-user`。測試 `api/tests/cli/test_provision_operator.py` 3 條（建 OAuth-only row / 每 operator 相異隨機 hash / 重複 email fail-loud），`ruff check` + `ruff format` + `mypy --strict` + `pytest tests/cli/` 6 條全綠。本單收掉 `planning/CLAUDE.md` §1 operator persona pass 的 T-068 下游缺口。
...
| T-070 | Vite dev server `/oauth/` proxy — OAuth 登入在 `localhost:5173` 壞掉：`web/vite.config.ts` dev proxy 只 proxy `/api`（且 target 指不到 containerized `pnpm dev` 的 api），沒有 `/oauth/` proxy → SPA 的 relative `/oauth/...` navigation 撞 Vite SPA fallback、bounce 回 `/login`。CI 看不到（e2e 走 nginx:80 同源）。T-068 dev 測試 reveal | TODO |
