...
| T-073 | Authentik source enrollment `next`-redirect 缺口 — 真人 operator 首登走 enrollment flow 完成後落在 `/if/user/`、不 redirect 回 SPA，重試又撞 `require_unauthenticated` 的 "Flow does not apply"。operator-amendment（補 `authentik-stack.md` §5.2）。T-070 dev 測試 wall 3 reveal | TODO |
