...
| T-078 | logout 後無法 re-login（wall 6）— SPA logout 只 revoke OAuth token、不結束 Authentik session → re-login 時 `default-source-authentication` 的 `require_unauthenticated` 判 "Flow does not apply" → 拒。T-073 早預告、T-076 後使用者實測確認。真功能 bug（連已 provision 的 operator 都中），優先級高於 T-077。| TODO |
