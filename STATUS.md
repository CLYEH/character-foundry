...
> ⚠ **開 M3.5 任何 ticket 之前必讀** `planning/agent-interface/scope.md` §5「規劃啟動順序」。M3.5 有 17 條未決 open-questions（9 + 8）彼此耦合，必須走完 4-step plan phase（agent-interface → auth parallel → backend review → frontend/devops downstream）才可以開 ticket，否則會邊做邊改大量返工。
