...
> **Last updated:** 2026-05-14 — T-068 implemented: SPA `/login` 從單一 Google 按鈕改成三入口（Google direct / 帳密 fallback / Dev escape hatch）。`web/src/lib/oauth-client.ts` 新增 `buildSourceInitUrl(authorizeUrl, sourceSlug)` — 把既有 `buildAuthorizeUrl` 的輸出整個包進 Authentik source-init redirect `/oauth/source/oauth/login/<slug>/?next=<encoded authorize url>`，讓 Google 路徑跳過 identification 中介頁直達 Google IdP；helper 從 authorize URL 抽 `/oauth/` 前綴（不另開 env var），slug 空字串回 `null`（caller 隱藏按鈕），authorize URL 缺 `/application/o/authorize/` 段則丟 `OauthError('invalid_authorize_url')`。`web/src/routes/login.tsx` 改三入口 layout：Google 主按鈕 + 帳密 fallback 按鈕（走原 `/oauth/application/o/authorize/` → Authentik identification + password）+ 角落 `Dev →` link（新分頁開 `/oauth/if/admin/`）；兩個 OAuth 入口共用同一份 `stashPkceState`，PKCE / state security posture 等同 T-056。新增 env var `VITE_AUTHENTIK_GOOGLE_SOURCE_SLUG`（`web/src/config.ts` `authentik.googleSourceSlug`，default `google`，留空關掉 shortcut），`.env.example` 同步。Authentik 2024.12 實測 `next` 吃 absolute 與 relative URL 皆可（curl 兩種都 302 到 Google）。測試：`oauth-client.test.ts` 補 `buildSourceInitUrl` 五條（relative / absolute / empty slug / unsafe-char slug encode / 缺段丟錯）、`login.test.tsx` 改三入口 render + click dispatch + 雙路徑 PKCE stash 斷言；e2e `oauth-login.spec.ts` 既有 password happy path 改走帳密 fallback 按鈕（fixture `oauthLoginViaUi` 同步），新增一條 Google entry 斷言（`page.waitForRequest` 抓 source-init hop URL 含 `/source/oauth/login/google/` + `next` 帶 authorize URL，不實走 Google，CI 無 upstream source）；`auth.spec.ts` 的 login-page 斷言改成三入口 + `getByRole('textbox')` count 0（證明無 inline form）。`planning/frontend/oauth-integration.md` §1 改寫成 dual-path + dev escape hatch，舊「單按鈕」決策保留於 §1.4 作歷史紀錄並寫明 operator persona pass 推翻理由（Google OAuth client Testing-mode refresh 7d 過期 → 需帳密 break-glass；setup / 改 Authentik 設定要 akadmin 進 admin UI → Dev link surface 這條 URL）。本單收掉 `planning/CLAUDE.md` §1 operator persona pass 的第一張範例單。
