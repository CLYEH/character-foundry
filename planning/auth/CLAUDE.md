...
> ⚠ **啟動順序提醒**：你不是 M3.5 第一個動的 agent——`agent-interface` agent 必須先拍板 MCP transport / tool 顆粒度 / async 體驗 / naming（`../agent-interface/open-questions.md` Q1-Q4），那 4 條決定 OAuth scope 顆粒度。詳見 `../agent-interface/scope.md` §5「規劃啟動順序」。
