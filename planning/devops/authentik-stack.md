...
> **T-089 — `character-foundry-mcp` 是 §5.4 第 6 個 app。** 真人 MCP client 的 auto-login（discovery）走它；設計全貌見 `../agent-interface/mcp-oauth-discovery.md`。e2e/CI 由 `infra/authentik/blueprints/cf-e2e-bootstrap.yaml` codify（含所有 silent-failure gotcha：`invalidation_flow` 必填、`redirect_uris` 用 `{matching_mode,url}` list、`per_provider` issuer、Self-signed signing key）；**dev 走 `infra/authentik/blueprints/cf-mcp-dev.yaml`（T-094，`docker-compose.override.yml` 掛進 authentik-server + worker，同 cf-google-init / cf-builtin-flow-hardening pattern）；prod 仍走本節 admin-UI 步驟或在 prod blueprint dir codify 同 shape**。建好後把它的 issuer 加進 `AUTHENTIK_ISSUER_URL`、client_id 加進 `AUTHENTIK_AUDIENCE`（兩者皆 CSV），否則 `/mcp/` 對它的 delegated token 回 `AUTH_CLIENT_NOT_ALLOWED`。⚠ ngrok 手測時 token 的 `iss` 由 Authentik 依 request Host（ngrok host）算出，要把 `https://<ngrok-host>/oauth/application/o/character-foundry-mcp/` 一併加進 `AUTHENTIK_ISSUER_URL`。⚠ **delegated client 必須拿得到 `email` claim**——PRM `scopes_supported` 已含 `openid/email/profile`（`app/mcp/discovery.py`，T-094），client request 後 Authentik 才發 `email`、backend `resolve_oauth_user_id` 才映射得到 `User`；少了會 `AUTH_INVALID_TOKEN`（T-094 dogfood 真因）。⚠ **顯式設 `MCP_ALLOWED_HOSTS` 會覆蓋 T-090 default 的 bare-host**：dev 經 nginx 進來 Host 是不帶 port 的 `localhost`，allowlist 必須含 bare `localhost`（不只 `localhost:*`），否則 authenticated 請求收 `421 Invalid Host header`。
...
- [ ] §5.4 **T-089 `character-foundry-mcp`**（第 6 個 app，真人 MCP delegated discovery）建好：public PKCE、explicit-consent、no `offline_access`、redirect_uris（loopback regex + claude.ai connector）、綁 `cf-agent-default` group。issuer 加進 `AUTHENTIK_ISSUER_URL`、client_id 加進 `AUTHENTIK_AUDIENCE`。**Apply path 依環境而異**——e2e/CI：`cf-e2e-bootstrap.yaml` 自動套；dev：`cf-mcp-dev.yaml`（T-094，`docker-compose.override.yml` 已掛單檔 mount，`docker compose up -d authentik-server authentik-worker` 即套，`BlueprintInstance.status == successful`）；prod：admin-UI（本節步驟）或 codify 進 prod blueprint dir。驗收：`curl <host>/.well-known/oauth-protected-resource` 回合法 RFC 9728（`authorization_servers` 指向此 app issuer、`scopes_supported` 含 `openid/email/profile` + 5 app scope）；無 token 打 `/mcp/` 回 `401 + WWW-Authenticate`；MCP Inspector / Claude Code OAuth 模式 end-to-end auto-login（AC #3，**T-094 2026-06-09 已用 Claude Code 跑通**：discovery → Google 登入 → `character.create` 建出 character+base、owner=真人）
