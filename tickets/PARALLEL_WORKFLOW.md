...
> - Wave D：**done**（T-037 squash-merged 2026-05-01 PR #62）
> - Wave E：unblocked（T-038 ready to start）
...
> ⚠ **CI 神祕停跑於某個 PR 時先查 conflict**：GitHub 對 `mergeable: CONFLICTING` 的 PR 靜默跳過 CI workflow（不會給任何 check signal）。Diagnostic command：`gh pr view N --json mergeable,mergeStateStatus`。看到 `CONFLICTING` / `DIRTY` 就知道要 rebase，不要先去追 billing / concurrency / workflow 假說。詳見 CLAUDE.md §6 merge gate 的「沒有任何 check」段。
