...
- `POST /v1/characters` — body `{ name, input_mode: 'template'|'reference' }` → 建 Character（尚無 base）+ 建 CreationSession（status=in_progress）+ 在 Redis SET `seq:checkpoint:{session_id} = 0`（T-017 的 sequence allocator 用），三步同 transaction（DB 失敗就 rollback；Redis 失敗 log warning 但不擋，T-017 fallback 會用 `MAX(sequence)+1` 重建）。回 `{ character, creation_session }`
...
- Validation：name 1–50 字（對齊 `planning/data/db-schema.md` line 103 `CHECK (char_length(name) BETWEEN 1 AND 50)` 與 `planning/product/data-model.md` line 67）、同 owner 下不可重名（回 `CONFLICT_DUPLICATE_NAME`）
