...
- `POST /v1/creation-sessions/{id}/checkpoints` — body：
  ...
  - 建 `checkpoints` row with status=queued（sequence = existing count + 1）
  - 透過 T-013 `TaskService.create_task('create_checkpoint', input_payload={checkpoint_id, session_id, mode, ...})` 排 arq job
  - 回 `{ task_id, checkpoint_id }`
...
- Cancel 支援：worker 每個階段結尾檢查 `task.cancel_requested`，若 true 則中止並 `status=cancelled`（配合 T-013）
...
- [ ] Cancel：queued 狀態 cancel → task=cancelled、checkpoint row 保留（status=cancelled 標記）
- [ ] 錯誤：reconciler fail / gpt-image-2 fail → task=failed + AgentError，checkpoint row 保留 `status=failed`
...
- Checkpoint row 先建後 enqueue：若 enqueue 失敗 → 把 checkpoint status 標 `failed`（已經有 row 比較好除錯）
