...
- `POST /v1/checkpoints/{checkpoint_id}/fork { new_character_name }` →
  ...
  - 新 session 裡塞第一個 checkpoint（sequence=1）：**複製 source checkpoint 的 image 到新 key** = `creation-sessions/{new_session_id}/checkpoints/{new_ckpt_id}.png`（含 `_thumb.png`），不能共用 source key — 因 source session 可能 abandoned 後 7 天被 cleanup 刪檔，新 character 的 checkpoint 會 broken image。`prompt` / `generation_log_id` 可共用 reference（純 metadata，不會被 cleanup）。Storage backend 提供 `copy(src_key, dst_key)` 介面（T-005 LocalFilesystemBackend 已有 file-copy 能力，OS-level 可用 hardlink 優化）
...
- [ ] Fork 產生新 character（新 owner = 呼叫者）+ 新 session + 第一 checkpoint 內容同源（image 已複製到新 key，prompt / generation_log_id 共用）
- [ ] 模擬 source session abandoned + cleanup → 確認 forked character 的 image 仍可讀（檔已複製）
