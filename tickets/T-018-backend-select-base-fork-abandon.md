...
- `POST /v1/creation-sessions/{id}/select-base { checkpoint_id }` →
  ...
  - 建 `bases` row（copy 圖 URL / from_checkpoint_id / generation_log）
  - 更新 Character `base_id`、`updated_at`、`thumbnail_url`
...
- `POST /v1/checkpoints/{checkpoint_id}/fork { new_character_name }` →
  ...
  - 新 session 裡塞第一個 checkpoint（sequence=1）複製自 source checkpoint（`output_image_url`、`prompt_summary`、`generation_log` copy；`forked_from_checkpoint_id` 指回來）
...
- `POST /v1/creation-sessions/{id}/abandon` →
  ...
  - Checkpoints 保留（30 天後清理 scheduled job 會處理，或先不處理 Phase 1）
  - 204 No Content
...
- `characters.thumbnail_url` 用 base image 的 thumbnail；select-base 時 copy 一份 URL reference
