...
- Serializer 邏輯：join `creation_sessions` 表，找該 character 的最新一筆；status='completed' 時可不回（傳 null）以節省 payload，或始終回（簡單一致）— 實作選後者
- DTO 同步補在 OpenAPI spec
- 單元測試：base 已確立 → null；in_progress session 存在 → 帶 id+status；abandoned → 帶 status='abandoned'
...
- [ ] `GET /v1/characters/{id}` Response 含 `creation_session` 欄位（in_progress / abandoned 時 id+status，completed 時 null）
