...
- 規則：
  - `app/api/routes/*` 不可直接 import `app/db/models/*`（要走 `app/repositories/` 或 `app/schemas/`）
...
- [ ] 三條規則跑得起來：api → models / ai → api / mcp ↔ auth scope source
...
- Error message LLM-friendly 範例：`"Forbidden import: 'app.api.routes.character' imports 'app.db.models.Character'. Routes must access models through 'app.repositories.character.*' (returns Pydantic schemas) — see CONTRIBUTING §x."`
